...
function drawStartMenu(gesture) {
  ...
  fill(200, 0, 255);
  text(
    'Púrpura: Signo de la paz (Daño Masivo), se activa tras usar Ao y Aka',
    startX,
    startY + lh * 3
  );
  fill(255);
  ...
  if (!audioEnabled) {
    ...
    textSize(20);
    text(
      '⚠️ HAZ CLICK EN LA PANTALLA UNA VEZ PARA ACTIVAR EL AUDIO ⚠️',
      width / 2,
      startY + lh * 6.5
    );
  } else {
...
function draw() {
  ...
  if (gesture !== 'NONE' && gesture !== 'FIST' && gesture !== 'COOLDOWN' && attackBox) {
    ...
    if (gesture === 'DOMAIN') {
      ...
      didFire = attackManager.tryAttack(
        'BASIC',
  ...
  if (getCurrentBot().hp <= 0) {
    ...
    fx.spawnParticles(
      getCurrentBot().x + getCurrentBot().w / 2,
      getCurrentBot().y + getCurrentBot().h / 2,
      100,
      'PURPLE'
    );
